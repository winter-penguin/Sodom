...
public class SoundManager : MonoBehaviour
{
	public struct Speaker	// BGM 을 재생하는 스피커인지, SFX를 재생하는 스피커인지 구분하기 위한 구조체
	{
		...
		public Speaker(AudioSource _speaker, SoundClipKinds _soundSort)
		{
			speaker = _speaker;
	...
	public enum SoundClipKinds { BGM, SFX }	// 해당 스피커가 재생하는 소리의 종류
	...
	public void Init()
	{
		if (Camera.main != null) mainSpeaker = Camera.main.gameObject.GetComponent<AudioSource>();	// 메인 카메라에 있는 오디오 소스 컴포넌트 가져오기
		SetSoundClips(soundClips, firstSoundIndex);
		
		speakers.Add(new Speaker(mainSpeaker, SoundClipKinds.BGM));	// BGM을 재생하는 스피커를 스피커 리스트에 추가
		
		Button[] tempButtons = FindObjectsOfType<Button>();	// SFX를 재생하는 버튼들을 가져오기
		for (var i = 0; i < tempButtons.Length; i++)	
		{
			speakers.Add(new Speaker(tempButtons[i].GetComponent<AudioSource>(), SoundClipKinds.SFX));	// SFX를 재생하는 버튼들의 스피커를 스피커 리스트에 추가
		}
		...
		for (var i = 0; i < speakers.Count; i++)
		{
			...
			{
				speakers[i].speaker.volume = PlayerPrefs.GetFloat("SFXVolume", defaultVolume);	// BGM 외에 SFX를 재생하는 스피커의 볼륨 설정
			}
		}
	}
		...
		switch (_soundKind)
		{
			...
			case SoundClipKinds.SFX:
				for (var i = 0; i < speakers.Count; i++)
				{
					if (speakers[i].soundSort == SoundClipKinds.SFX)
					{
						speakers[i].speaker.volume = _volume;
					}
				}
